...
HARD_PROB = [
    ...
    "fome12_Mittelman", "rail507_Mittelman", "rail582_Mittelman",
]
MIPLIB_LARGE_FILE = [
    "neos-3402454-bohle_MIPLIB", "square41_MIPLIB",
    "square47_MIPLIB", "neos-2075418-temuka_MIPLIB"
]
MITTELMAN_LARGE_FILE = [
    "neos-3025225_Mittelman", "s82_Mittelman",
    "Primal2_1000_Mittelman", "thk_48_Mittelman",
    "L2CTA3D_Mittelman", "tpl-tub-ws1617_Mittelman",
    "netlarge1_Mittelman", "thk_63_Mittelman",
    "fhnw-binschedule1_Mittelman", "a2864_Mittelman",
    "netlarge6_Mittelman", "Dual2_5000_Mittelman",
    "dlr2_Mittelman", "dlr1_Mittelman", 
    "square41_Mittelman", "bharat_Mittelman",
    "scpm1_Mittelman", "netlarge3_Mittelman",
    "set-cover-model_Mittelman", "supportcase19_Mittelman",
]
NETLIB_LARG_FILES = []
EPS_ABS         = 1e-6
EPS_REL         = 1e-6
...
FIRST_50_OSQP_RUN_TIME  = None
FIRST_100_OSQP_RUN_TIME = None
FIRST_150_OSQP_RUN_TIME = None
FIRST_200_OSQP_RUN_TIME = None
FIRST_250_OSQP_RUN_TIME = None
...
def get_problem_path(problem_name):
    ...
    if problem_name.endswith('_Maros_Meszaros'):
        ...
        data_dir = os.path.join(script_dir, "..", "..", "NetLib_mat")
        if not problem_name.endswith('.mat'):
            problem_name = problem_name + '.mat'
        ...
        # if not problem_name.endswith('.mps'):
        #     problem_name = problem_name + '.mps'
        data_dir = os.path.join(script_dir, "..", "..", "Mittelman_mat")
        if not problem_name.endswith('.mat'):
            problem_name = problem_name + '.mat'
        ...
        # if not problem_name.endswith('.mps'):
        #     problem_name = problem_name + '.mps'
        data_dir = os.path.join(script_dir, "..", "..", "MIPLIB_mat")
        if not problem_name.endswith('.mat'):
            problem_name = problem_name + '.mat'
...
def problem_solver(
    ...
    total_run_time      = 0.0
    first_50_run_time   = 0.0
    first_100_run_time  = 0.0
    first_150_run_time  = 0.0
    first_200_run_time  = 0.0
    first_250_run_time  = 0.0
    
    first_50_osqp_time_ratio_sum   = 0.0
    first_100_osqp_time_ratio_sum  = 0.0
    first_150_osqp_time_ratio_sum  = 0.0
    first_200_osqp_time_ratio_sum  = 0.0
    first_250_osqp_time_ratio_sum  = 0.0
    ...
    for path in PATHS:
        ...
        prob.setup(P, q, A, l, u, plot=PLOT, verbose=False, eps_abs=EPS_ABS, eps_rel=EPS_REL, eps_prim_inf=EPS_PRIM_INF, eps_dual_inf=EPS_DUAL_INF, time_limit=360, max_iter=2000000000, **current_comb)
        ...
        if (prob_counter < 50) and (prob_counter != prob_counter_50):
            print(f"One of the first 50 problems tested is not in our PURE_OSQP_DF", flush=True)
            raise
        ...
        # if (res.info.run_time >= cut_off_multiple * (osqp_df_cur_prob['run time'].iloc[0] + 0.5)):
        ...
        try:
            ...
            with open(stat_file, mode='a', newline='', encoding='utf8') as csvfile:
                ...
                try:
                    ...
                    # If lock fails immediately, wait and retry
                    delay = base_delay * (2 * attempt) + random.uniform(0, 0.1)
                    time.sleep(delay)
                    continue
                
                try:
                    # Re-check if we need header after acquiring lock
                    csvfile.seek(0, 2)  # Seek to end
                    file_size = csvfile.tell()
                    needs_header = file_size == 0
                    
                    writer = csv.DictWriter(csvfile, fieldnames=stats_data.keys())
                    if needs_header or file_is_empty:
                        writer.writeheader()
                    writer.writerow(stats_data)
            ...
            if attempt == max_retries - 1:
                print(f"Failed to write to {stat_file} after {max_retries} attempts: {e}", flush=True)
                raise
            else:
...
def objective(trial):
    ...
    restart_type_int = trial.suggest_categorical("restart_type", [int(osqp.ext_builtin.OSQP_RESTART_REFLECTED_HALPERN)])
    ...
    # if current_comb["restart_type"] == osqp.ext_builtin.OSQP_RESTART_REFLECTED_HALPERN:
    ...
    if use_multiprocessing:
        with multiprocessing.Pool(processes=num_cpus) as pool:
            if CLUSTER == "della_stellato":
                ...
                # results = pool.map(run_optimization, range(72))
                # results = pool.map(run_optimization, range(num_cpus*400))
...
if __name__ == '__main__':
    try:
        ...
        if PROBLEM_NAME != "RANDOM":
            ...
            if (PROBLEM_NAME == "ALL.mat"):                
                probs_above_100_sec = ['CVXQP3_L_Maros_Meszaros', 'QPILOTNO_Maros_Meszaros', 'cmflsp50-24-8-8_MIPLIB', 
                                       'neos-3555904-turama_MIPLIB', 'neos-3988577-wolgan_MIPLIB',
                                       'fome11_Mittelman', 'boeing1_NetLib', 'pilotnov_NetLib',
                                       'stair_NetLib']  
                ...
                exclude_probs = exclude_probs + [f"{p}.mps" for p in HARD_PROB] + [f"{p}.mat" for p in HARD_PROB]

                exclude_probs = exclude_probs + [f"{p}.mps" for p in LONG_RUNNING_PROBLEMS] + [f"{p}.mat" for p in LONG_RUNNING_PROBLEMS]
                exclude_probs = exclude_probs + [f"{p}.mps" for p in MIPLIB_LARGE_FILE] + [f"{p}.mat" for p in MIPLIB_LARGE_FILE]
                exclude_probs = exclude_probs + [f"{p}.mps" for p in MITTELMAN_LARGE_FILE] + [f"{p}.mat" for p in MITTELMAN_LARGE_FILE]
                exclude_probs = exclude_probs + [f"{p}.mps" for p in NETLIB_LARG_FILES] + [f"{p}.mat" for p in NETLIB_LARG_FILES]
                exclude_probs = exclude_probs + [f"{p}.mps" for p in probs_above_100_sec] + [f"{p}.mat" for p in probs_above_100_sec]

                files_qp = [f for f in os.listdir(qpbenchmark_data_dir) if (f.endswith('.mat')) and (f not in exclude_probs)]
                files_qp.sort()
                files_miplib = [f for f in os.listdir(MIPLIB_data_dir) if (f.endswith('.mat')) and (f not in exclude_probs)]
                files_miplib.sort()
                files_mittelman = [f for f in os.listdir(Mittelman_data_dir) if (f.endswith('.mat')) and (f not in exclude_probs)]
                files_mittelman.sort()
                files_netlib = [f for f in os.listdir(NetLib_data_dir) if (f.endswith('.mat')) and (f not in exclude_probs)]
                files_netlib.sort()
                ...
                PATHS = (
                    ...
                    # [os.path.join(qpbenchmark_data_dir, "AUG2D_Maros_Meszaros.mat")]
                    # [os.path.join(qpbenchmark_data_dir, f) for f in files_qp]
                )
        ...
        stat_file = "../../osqp/plot/stats_integral_Maros_MIPLIB_Mittelman_all_easy_probs_mat.csv"
        PURE_OSQP_DF = pd.read_csv(stat_file)
        TOTAL_OSQP_RUN_TIME     = PURE_OSQP_DF['run time'].sum()
        FIRST_50_OSQP_RUN_TIME  = PURE_OSQP_DF['run time'].iloc[:50].sum()
        FIRST_100_OSQP_RUN_TIME = PURE_OSQP_DF['run time'].iloc[:100].sum()
        FIRST_150_OSQP_RUN_TIME = PURE_OSQP_DF['run time'].iloc[:150].sum()
        FIRST_200_OSQP_RUN_TIME = PURE_OSQP_DF['run time'].iloc[:200].sum()
        FIRST_250_OSQP_RUN_TIME = PURE_OSQP_DF['run time'].iloc[:250].sum()
        
        print(f"TOTAL_OSQP_RUN_TIME: {TOTAL_OSQP_RUN_TIME}")
        print(f"FIRST_50_OSQP_RUN_TIME: {FIRST_50_OSQP_RUN_TIME}")
        print(f"FIRST_100_OSQP_RUN_TIME: {FIRST_100_OSQP_RUN_TIME}")
        print(f"FIRST_150_OSQP_RUN_TIME: {FIRST_150_OSQP_RUN_TIME}")
        print(f"FIRST_200_OSQP_RUN_TIME: {FIRST_200_OSQP_RUN_TIME}")
        print(f"FIRST_250_OSQP_RUN_TIME: {FIRST_250_OSQP_RUN_TIME}")
        ...
        if CLUSTER == "della_stellato":
            ...
            print(f"This is for solving {len(PATHS)} instances")
            ...
            msg['From'] = 'emailsender062@gmail.com'
            msg['To'] = 'lb3825@princeton.edu'
            msg.set_content(f'Job Finished !!!! It took {(end - START_TIMER) / 60.0} minutes. We solved a total of {len(PATHS)} problems')
