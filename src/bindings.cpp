...
PYBIND11_MODULE(ext_builtin, m) {
    ...
    .def_property("restart_type",
        [](const OSQPSettings& s) { return std::string(s.restart_type); },
        [](OSQPSettings& s, const std::string& value) {
            strncpy(s.restart_type, value.c_str(), OSQP_MAX_RESTART_TYPE_LEN - 1);
            s.restart_type[OSQP_MAX_RESTART_TYPE_LEN - 1] = '\0';
        }
    )
    .def_property("halpern_scheme",
        [](const OSQPSettings& s) { return std::string(s.halpern_scheme); },
        [](OSQPSettings& s, const std::string& value) {
            strncpy(s.halpern_scheme, value.c_str(), OSQP_MAX_HALPERN_SCHEME_LEN - 1);
            s.halpern_scheme[OSQP_MAX_HALPERN_SCHEME_LEN - 1] = '\0';
        }
    )
